<template>
  <div class="board-game-list-view">
    ...
    <div class="controls list-controls">
      ...
      <div class="filter-item">
        <span>Displaying {{ processedGames.length }} of {{ boardGameStore.games.length }} games</span> <!-- Use processedGames here -->
      </div>
    </div>
    ...
    <ul v-else-if="processedGames.length > 0" class="game-list"> <!-- Iterate over processedGames -->
      <li v-for="game in processedGames" :key="game.id" class="game-item" :class="[{ 'is-expansion-item': game.isExpansion }, statusClass(game.status)]">
        <div class="game-thumbnail">
          <router-link :to="{ name: 'GameDetail', params: { id: game.id } }">
            <img
              v-if="game.userImageUrls && game.userImageUrls.length > 0"
              :src="game.userImageUrls[0]"
              :alt="game.name + ` user thumbnail`"
            />
            <img
              v-else-if="game.thumbnailUrl"
              :src="game.thumbnailUrl"
              :alt="game.name + ` BGG thumbnail`"
            />
    ...
    <p v-else-if="boardGameStore.games.length > 0 && processedGames.length === 0">No games match filters.</p>
    <p v-else class="no-games-message">Collection is empty.</p>

    <!-- Log Play Modal (Full structure from previous step) -->
...
const filteredGames = computed(() => { /* ... (Full filter logic from previous step) ... */
  let games = boardGameStore.games;
  if (selectedStatus.value) { games = games.filter(game => game.status === selectedStatus.value); }
  if (searchQuery.value.trim() !== "") { const lq = searchQuery.value.toLowerCase(); games = games.filter(g=>(g.name.toLowerCase().includes(lq))||(g.description && g.description.toLowerCase().includes(lq))); }
  if (filterNumPlayers.value && filterNumPlayers.value > 0) { games = games.filter(g=>g.minPlayers!==undefined&&g.maxPlayers!==undefined&&g.minPlayers<=filterNumPlayers.value!&&g.maxPlayers>=filterNumPlayers.value!); }
  if (filterExpansionStatus.value === "base") { games = games.filter(g => !g.isExpansion); } else if (filterExpansionStatus.value === "expansion") { games = games.filter(g => g.isExpansion === true); }
  return games;
});
const processedGames = computed(() => { /* ... (Full sorting logic from previous step) ... */
  const gamesToSort = [...filteredGames.value];
  gamesToSort.sort((a,b)=>{let valA:any,valB:any;switch(sortBy.value){case"bggRating":case"bggComplexity":case"yearPublished":valA=a[sortBy.value as keyof BoardGame]??(sortDirection.value==="asc"?Infinity:-Infinity);valB=b[sortBy.value as keyof BoardGame]??(sortDirection.value==="asc"?Infinity:-Infinity);break;case"lastPlayedDate":valA=a.plays&&a.plays.length>0?new Date(a.plays[0].date).getTime():(sortDirection.value==="asc"?Infinity:-Infinity);valB=b.plays&&b.plays.length>0?new Date(b.plays[0].date).getTime():(sortDirection.value==="asc"?Infinity:-Infinity);break;case"playCount":valA=a.plays?.length??0;valB=b.plays?.length??0;break;default:valA=a.name.toLowerCase();valB=b.name.toLowerCase();}if(valA<valB)return sortDirection.value==="asc"?-1:1;if(valA>valB)return sortDirection.value==="asc"?1:-1;if(sortBy.value!=="name"){return a.name.toLowerCase()<b.name.toLowerCase()?-1:1}return 0;});
  return gamesToSort;
});
...
.list-controls { margin-bottom: 20px; display:flex; gap:15px; align-items:center; background-color:#f8f9fa; padding:10px 15px; border-radius:4px; flex-wrap: wrap; }
...
.filter-item input[type="text"], .filter-item select, .filter-item input[type="number"] { padding: 6px 8px; border-radius: 3px; border: 1px solid #ccc; font-size:0.9em; }
...
.game-thumbnail { width:100%; height:180px; overflow:hidden; background-color: #f0f0f0; display: flex; align-items: center; justify-content: center; }
.game-thumbnail img { width:100%; height:100%; object-fit:cover; }
.no-image-placeholder { color: #aaa; font-size: 0.9em; }
.game-info { padding: 15px; flex-grow: 1; display: flex; flex-direction: column; }
.game-info h3 { margin:0 0 8px 0; font-size:1.3em; } .game-info h3 a { text-decoration:none; color: #333; }
...
.status-owned { background-color: #28a745 !important; } .status-wishlist { background-color: #ffc107 !important; color: #212529 !important;}
...
.edit-btn-list { background-color:#ffc107; color:#212529; } .delete-btn-list { background-color:#dc3545; }
...
.modal-content h3{margin-top:0;margin-bottom:20px;color:#333} .modal-content > div {margin-bottom:15px;}
...
.modal-button-submit { background-color:#007bff; color:white; } .modal-button-cancel { background-color:#6c757d; color:white; }
