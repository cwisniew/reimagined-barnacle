...
import { BoardGame, OtherLink, CardSet, PlaySession, BggVideoLink, PlayerInPlaySession } from "./types";
...
const loadGames = () => {
  try { if (fs.existsSync(DATA_FILE)) { const d = fs.readFileSync(DATA_FILE, "utf-8"); if(d){ const p = JSON.parse(d); boardGames = Array.isArray(p)?p:[];
    boardGames.forEach(g=>{
      ...
      else if (Array.isArray(g.plays)) { g.plays.forEach(play => { if ((play as any).playerNames && !play.playersInSession) { play.playersInSession = ((play as any).playerNames as string[]).map(name => ({ id: uuidv4(), name: name })); delete (play as any).playerNames; } else if (!play.playersInSession) { play.playersInSession = []; } }); }
      ...
      if(g.cardSets===undefined)g.cardSets=[]; delete (g as any).bggVideoLinks;
      if(g.userImageUrls===undefined)g.userImageUrls=[]; // Initialize userImageUrls
    }); console.log("Loaded games, initialized userImageUrls."); } else { boardGames=[]; console.log(DATA_FILE, "is empty.");}} else { boardGames=[]; console.log(DATA_FILE, "not found.");}
...
app.post("/api/boardgames", async (req: Request, res: Response) => {
  const { name, description, /* ... other fields ... */ userImageUrls } = req.body as Partial<BoardGame>;
  if (!name || !description) return res.status(400).json({ message: "Name and description are required fields."});

  const gameDataForSave = { ...req.body };
  delete (gameDataForSave as any).bggVideoLinks;
  ...
  const newGame: BoardGame = {
    ...
    name,
    description,
    plays: gameDataForSave.plays || [],
    cardSets: gameDataForSave.cardSets || [],
...
app.put("/api/boardgames/:id", async (req: Request, res: Response) => {
  ...
  delete (updateData as any).bggVideoLinks;

  if (gameData.hasOwnProperty("userImageUrls")) {
      updateData.userImageUrls = gameData.userImageUrls || [];
  }
