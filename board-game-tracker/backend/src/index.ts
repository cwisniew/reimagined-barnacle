...
const loadGames = () => {
  try { if (fs.existsSync(DATA_FILE)) { const d = fs.readFileSync(DATA_FILE, "utf-8"); if(d){ const p = JSON.parse(d); boardGames = Array.isArray(p)?p:[];
    boardGames.forEach(g=>{
      if ((g as any).manualUrl && g.onlineManualUrl === undefined) { // Migration
        g.onlineManualUrl = (g as any).manualUrl;
      }
      delete (g as any).manualUrl; // Remove old field
      if(g.localManualUrl===undefined) g.localManualUrl = undefined;

      // Other initializations from previous steps
      ...
      if(g.bggReimplementations===undefined)g.bggReimplementations=[]; if(g.bggVideoLinks===undefined)g.bggVideoLinks=[];
    }); console.log("Loaded games, migrated manualUrl, initialized fields."); } else { boardGames=[]; console.log(DATA_FILE, "is empty.");}} else { boardGames=[]; console.log(DATA_FILE, "not found.");}
...
const gameImageStorage = multer.diskStorage({ destination:(req,f,cb)=>{const up=path.join(__dirname,"..","uploads","game-images");fs.mkdirSync(up,{recursive:true});cb(null,up)}, filename:(req,f,cb)=>{ const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9); cb(null, f.fieldname + '-' + uniqueSuffix + path.extname(f.originalname));} });
...
const manualFileStorage = multer.diskStorage({ destination:(req,f,cb)=>{const p=path.join(__dirname,"..","uploads","game-manuals");fs.mkdirSync(p,{recursive:true});cb(null,p)}, filename:(req,f,cb)=>{ const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9); cb(null, f.fieldname + '-' + uniqueSuffix + path.extname(f.originalname));}});
const manualFileFilter=(req:Request,f:Express.Multer.File,cb:multer.FileFilterCallback)=>{const at=["application/pdf","application/msword","application/vnd.openxmlformats-officedocument.wordprocessingml.document","text/plain"];if(at.includes(f.mimetype))cb(null,true);else cb(new Error("Invalid file type. Only PDF, DOC, DOCX, TXT allowed for manuals."))};
...
app.post("/api/boardgames", async (req: Request, res: Response) => {
  const { name, description, version, bggId, status, bggRating, bggComplexity, yearPublished, minPlayers, maxPlayers, playingTime, thumbnailUrl, imageUrl, isExpansion, baseGameAppId, bggBaseGameId, bggExpansionIds, designers, publishers, categories, mechanics, locationRoom, locationCupboard, locationShelf, locationNotes, officialWebsiteUrl, otherLinks, crowdfundingPlatform, crowdfundingUrl, crowdfundingStatus, bggSubdomains, bggFamilies, onlineManualUrl, localManualUrl, cardSets, plays, userImageUrls, bggVideoLinks, bggReimplementations } = req.body as Partial<BoardGame>;
  if (!name || !description) return res.status(400).json({ message: "Name and description are required fields."});

  const gameDataForSave: Omit<BoardGame, "id"> = { name, description, version, bggId, status, bggRating, bggComplexity, yearPublished, minPlayers, maxPlayers, playingTime, thumbnailUrl, imageUrl, userImageUrls: userImageUrls || [], plays: plays || [], isExpansion: isExpansion || false, baseGameAppId, bggBaseGameId, bggExpansionIds: bggExpansionIds || [], designers: designers || [], publishers: publishers || [], categories: categories || [], mechanics: mechanics || [], locationRoom, locationCupboard, locationShelf, locationNotes, officialWebsiteUrl, otherLinks: otherLinks || [], crowdfundingPlatform, crowdfundingUrl, crowdfundingStatus, bggSubdomains: bggSubdomains || [], bggFamilies: bggFamilies || [], onlineManualUrl, localManualUrl, cardSets: cardSets || [], bggReimplementations: bggReimplementations || [], bggVideoLinks: bggVideoLinks || [] };
  const newGame: BoardGame = { id: uuidv4(), ...gameDataForSave };
...
app.put("/api/boardgames/:id", async (req: Request, res: Response) => {
  ...
  if (gameData.hasOwnProperty("onlineManualUrl")) updateData.onlineManualUrl = gameData.onlineManualUrl; else if (boardGames[gameIndex].onlineManualUrl && !gameData.hasOwnProperty("onlineManualUrl")) updateData.onlineManualUrl = boardGames[gameIndex].onlineManualUrl;
  if (gameData.hasOwnProperty("localManualUrl")) updateData.localManualUrl = gameData.localManualUrl; else if (boardGames[gameIndex].localManualUrl && !gameData.hasOwnProperty("localManualUrl")) updateData.localManualUrl = boardGames[gameIndex].localManualUrl;

  // Ensure array fields are fully replaced or initialized if provided as undefined/null
  const arrayFields: (keyof BoardGame)[] = ["userImageUrls", "bggReimplementations", "bggVideoLinks", "otherLinks", "cardSets", "plays", "bggExpansionIds", "designers", "publishers", "categories", "mechanics", "bggSubdomains", "bggFamilies"];
  arrayFields.forEach(field => {
    if (gameData.hasOwnProperty(field)) {
      (updateData as any)[field] = (gameData as any)[field] || [];
    }
  });
...
app.get("/api/bgg/game/:bggId", async(req,res)=>{
  ...
  try{ const tr = await bggClient.thing.query({id:[idNum],stats:1,versions:1,videos:1}); if(!tr||!tr.length) return res.status(404).json({message:"Game not found on BGG"});
    ...
    if(gd.links&&Array.isArray(gd.links)){gd.links.forEach((l:Link)=>{switch(l.type){ case"boardgameexpansion":if(l.inbound==="true"){isExp=true;bggBaseId=l.id}else{bggExpIds.push(l.id)}break; case"boardgamedesigner":des.push(l.value);break;case"boardgamepublisher":p.push(l.value);break; case"boardgamecategory":cat.push(l.value);break;case"boardgamemechanic":mec.push(l.value);break; case"boardgamesubdomain":subdom.push(l.value);break;case"boardgamefamily":fam.push(l.value);break; case"reimplementation": reimplementationsFromBgg.push({ bggId: l.id, name: l.value }); break;}});}
    ...
    const augDetails={...gd,_isExpansionFromBgg:isExp,_bggBaseGameIdFromBgg:bggBaseId,_bggExpansionIdsFromBgg:bggExpIds,_designers:des,_publishers:pub,_categories:cat,_mechanics:mec,_officialWebsiteFromBgg:webUrl,_bggSubdomains:subdom,_bggFamilies:fam,_bggVideos:vidsFromBgg, _bggReimplementations:reimplementationsFromBgg};
    res.json(augDetails);
